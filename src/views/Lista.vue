  ...
  <div class="container mt-2">
    <div v-for="(task, index) in tasks" :key="index">
      <b-card :title="task.subject" class="mb-2">
        <b-card-text>Nome do Produto: {{ task.subject }}</b-card-text>
        <b-card-text>Código do Produto: {{ task.codigo_subject }}</b-card-text>
        <b-card-text>Valor do Produto: {{ task.valor }}</b-card-text>
         <b-card-text>Descrição do Produto: {{ task.description }}</b-card-text>
    ...
    <b-modal ref="modalRemove" hide-footer title="Exclusão de tarefa">
      <div class="d-block-text-center">Deseja excluir esta tarefa? {{ taskSelected.subject }}
